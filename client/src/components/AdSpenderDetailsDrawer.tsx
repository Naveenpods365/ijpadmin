...
import type { AdSpenderRow } from "@/components/AdSpenderDetailsPopup";

type AdSpenderDetailsDrawerProps = {
    open: boolean;
...
export function AdSpenderDetailsDrawer({
    open,
    vendor,
    onClose,
    onCloseAd,
}: AdSpenderDetailsDrawerProps) {
    useEffect(() => {
        if (!open) return;
    ...
    const barData = useMemo(() => {
        if (!vendor) return [];
        const base = Math.max(12, Math.round(vendor.ctr * 8));
        const days = [
            "20 jun",
            "21 jun",
            "22 jun",
        ...
        return days.map((d, idx) => ({
            day: d,
            value: Math.max(8, Math.round(base + idx * 5 + (vendor.spend % 9))),
        }));
    }, [vendor]);
    ...
    const reach = useMemo(() => {
        if (!vendor) return 0;
        return Math.round(93000 + vendor.spend * 1.7);
    }, [vendor]);

    const donutPct = useMemo(() => {
        const total = 120000;
        const reached = Math.min(total, reach);
        return total ? reached / total : 0;
    }, [reach]);
    ...
    const BarWithBubble = (barProps: any) => {
        const { x, y, width, height, fill, index } = barProps;
        ...
        <AnimatePresence>
            {open && vendor ? (
                ...
                >
                    ...
                    <motion.div
                        className="absolute inset-y-0 right-0 w-[90vw] max-w-[980px] bg-white"
                        initial={{ x: "100%" }}
                        animate={{ x: 0 }}
                        exit={{ x: "100%" }}
                        transition={{
                    ...
                    >
                        <div className="h-full flex flex-col">
                            <div className="h-[84px] px-8 flex items-center justify-between border-b border-[#edf1f3]">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="w-[56px] h-[56px] rounded-full overflow-hidden bg-[#f6f8fa] border border-[#edf1f3]">
                                            <img
                                        ...
                                        <div className="leading-tight">
                                            <div className="text-[22px] font-semibold text-[#222f36] leading-[24px]">
                                                Siti Arlina
                                            </div>
                                            <div className="mt-2 flex items-center gap-2">
                                                <span className="inline-flex items-center h-6 rounded-full px-3 text-[11px] font-semibold bg-[#e9f0ff] text-[#2b73f7] gap-1">
                                ...
                                <div className="flex items-center gap-3">
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button
                                                type="button"
                                                className="h-10 w-10 rounded-[10px] bg-white border border-[#edf1f3] flex items-center justify-center hover:bg-[#f6f8fa]"
                                                aria-label="Menu"
                                            >
                                        ...
                                        <DropdownMenuContent
                                            align="end"
                                            className="w-[200px] rounded-[12px] border border-[#edf1f3] shadow-[0px_12px_30px_rgba(0,0,0,0.10)] p-2"
                                        >
                                            <DropdownMenuItem
                                                className="cursor-pointer rounded-[10px] px-3 py-2 text-[#7b848f] focus:bg-[#f6f8fa] focus:text-[#222f36]"
                                                onSelect={() => onCloseAd()}
                                            >
                                                <TriangleAlert className="w-4 h-4 mr-2 text-[#7b848f]" />
                                                Close Add
                                            </DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                    ...
                                    <button
                                        type="button"
                                        className="text-[#16a249] text-[12px] font-semibold hover:underline"
                                    >
                                        View Profile
                                    </button>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto">
                                <div className="p-8">
                                    ...
                                    <div className="mt-4 rounded-[14px] border border-[#edf1f3] bg-white overflow-hidden">
                                        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px]">
                                            <div className="relative">
                                                ...
                                                <div className="absolute left-4 bottom-4 h-9 px-3 rounded-full bg-black/35 text-white text-[11px] flex items-center gap-2">
                                                    93,050 Views
                                                </div>
                                            </div>

                                            <div className="p-5 border-l border-[#edf1f3]">
                                                ...
                                                <div className="mt-4 space-y-3 text-[12px]">
                                                    {[
                                                        ...
                                                        >
                                                            ...
                                                            <div className="text-[#62a230] font-semibold">
                                                                {r.value}
                                                            </div>
                                                        </div>
                                                    ))}
                                        ...
                                        <div className="h-[54px] border-t border-[#edf1f3] flex items-center justify-center gap-4">
                                            <button
                                                type="button"
                                                className="h-9 w-9 rounded-full bg-[#f6f8fa] flex items-center justify-center"
                                            >
                                                <ChevronLeft className="h-4 w-4 text-[#7b848f]" />
                                            </button>
                                            <div className="flex items-center gap-4 text-[11px] text-[#7b848f]">
                                                <span className="h-7 w-7 rounded-full bg-[#62a230] text-white flex items-center justify-center font-semibold">
                                                    01
                                                </span>
                                    ...
                                    <div className="mt-8 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10">
                                        <div>
                                            ...
                                            <div className="mt-4">
                                                ...
                                                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-2 text-[12px]">
